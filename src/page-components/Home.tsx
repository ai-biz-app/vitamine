...
export default function Home() {
  ...
  return (
    <div className="relative">
      <SEO
        title="ClearlyVitamins — Essential Vitamin Guide"
        description="Cut through the supplement noise. Get clear, science-backed answers about the 13 vitamins your body can't live without."
      ...
      <main>
        ...
        <section ref={heroRef} className="w-full flex flex-col items-center justify-center text-center px-6" style={{ minHeight: '85vh', paddingTop: 80 }}>
          ...
          <div className="hero-cta flex gap-4 mt-10 flex-wrap justify-center">
            <Link href="/supplement-guide" className="px-8 py-3.5 rounded-full text-[14px] font-medium text-white transition-all duration-300 hover:opacity-80" style={{ fontFamily: "'Geist Sans', sans-serif", background: '#1a1a1a' }}>
              {t('hero.cta1')} →
            </Link>
            <Link href="/vitamins" className="px-8 py-3.5 rounded-full text-[14px] font-medium transition-all duration-300 hover:opacity-80" style={{ fontFamily: "'Geist Sans', sans-serif", color: '#1a1a1a', border: '1px solid rgba(0,0,0,0.2)' }}>
              {t('hero.cta2')}
            </Link>
          </div>
        </section>
        ...
        <section className="w-full section-padding" style={{ background: '#f5f5f0' }}>
          <div className="content-max px-6">
            ...
            <div ref={startRef} className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <Link href="/vitamins" className="group block rounded-xl p-8 transition-all duration-300 hover:-translate-y-1" style={{ background: '#ffffff', boxShadow: '0 4px 16px rgba(0,0,0,0.1)', border: '1px solid rgba(0,0,0,0.12)' }}
              ...
              <Link href="/supplement-guide" className="group block rounded-xl p-8 transition-all duration-300 hover:-translate-y-1" style={{ background: '#ffffff', boxShadow: '0 4px 16px rgba(0,0,0,0.1)', border: '1px solid rgba(0,0,0,0.12)' }}
              ...
              {/* Card 3: Deficiencies */}
              <Link href="/deficiencies" className="group block rounded-xl p-8 transition-all duration-300 hover:-translate-y-1" style={{ background: '#ffffff', boxShadow: '0 4px 16px rgba(0,0,0,0.1)', border: '1px solid rgba(0,0,0,0.12)' }}
        ...
        <section className="w-full section-padding" style={{ background: '#fafaf8' }}>
          <div className="content-max px-6">
            ...
            <div ref={statsRef} className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { num: '13', label: t('stat.essential'), color: '#4ade80', href: '/vitamins' },
                { num: '4', label: t('stat.fat'), color: '#fb923c', href: '/vitamins' },
                { num: '9', label: t('stat.water'), color: '#60a5fa', href: '/vitamins' },
                { num: '1B+', label: t('stat.deficient'), color: '#fb7185', href: null, source: 'https://ods.od.nih.gov/factsheets/VitaminD-HealthProfessional/' },
              ].map((s) => (
                <div key={s.label} className={`text-center rounded-xl p-6 bg-white ${s.href ? 'cursor-pointer transition-all duration-200 hover:shadow-md' : ''}`} style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
                  {s.href ? (
                  ...
                  {s.source && (
                    <a
                      ...
                      className="inline-block mt-2 text-[11px] transition-colors hover:underline"
                      style={{ fontFamily: "'Geist Mono', monospace", color: '#4a4a4a', opacity: 0.5 }}
                    >
                      {lang === 'ko' ? '출처: NIH' : 'Source: NIH'} ↗
                    </a>
                  )}
                </div>
              ))}
              ...
              <div className="rounded-xl p-8" style={{ background: 'rgba(96, 165, 250, 0.06)', border: '1px solid rgba(96, 165, 250, 0.12)' }}>
                ...
                <div className="flex flex-wrap gap-2 mt-5">
                  {waterSolubleVits.map((v) => (
                    <Link key={v.label} href={v.path} className="px-3 py-1.5 rounded-lg text-[13px] font-semibold transition-all duration-150 hover:scale-110" style={{ fontFamily: "'Geist Sans', sans-serif", background: 'rgba(96, 165, 250, 0.1)', color: '#3b82f6' }}>
                      {v.label}
                    </Link>
                  ))}
                </div>
              ...
              <div className="rounded-xl p-8" style={{ background: 'rgba(251, 146, 60, 0.06)', border: '1px solid rgba(251, 146, 60, 0.12)' }}>
                ...
                <div className="flex flex-wrap gap-2 mt-5">
                  {fatSolubleVits.map((v) => (
                    <Link key={v.label} href={v.path} className="px-3 py-1.5 rounded-lg text-[13px] font-semibold transition-all duration-150 hover:scale-110" style={{ fontFamily: "'Geist Sans', sans-serif", background: 'rgba(251, 146, 60, 0.1)', color: '#ea580c' }}>
                      {v.label}
                    </Link>
                  ))}
                </div>
        ...
        <section className="w-full py-24 px-6" style={{ background: '#1a1a1a' }}>
          <div className="content-max text-center" ref={ctaRef}>
            ...
            <div className="flex gap-4 justify-center mt-10 flex-wrap">
              <Link href="/supplement-guide" className="px-8 py-3.5 rounded-full text-[14px] font-medium transition-all duration-300 hover:opacity-80" style={{ fontFamily: "'Geist Sans', sans-serif", background: '#fafaf8', color: '#1a1a1a' }}>
                {t('stats.cta.btn')} →
              </Link>
              <Link href="/vitamins" className="px-8 py-3.5 rounded-full text-[14px] font-medium text-white transition-all duration-300 hover:opacity-80" style={{ fontFamily: "'Geist Sans', sans-serif", border: '1px solid rgba(250,250,248,0.3)' }}>
                {t('ql.vitamins')}
              </Link>
            </div>
            <div className="mt-8">
