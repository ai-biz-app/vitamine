...
import { vitaminPageSchema, breadcrumbSchema } from '../lib/seoSchema'
...
export default function VitaminDetail() {
  ...
  const seoTitle = `${v?.name || t('vd.notFound')} — Functions, Sources, RDA`
  const seoDesc = v?.description || t('vd.notFound.sub')
  const pageSchema = useMemo(() => {
    if (!v || !vitamin) return undefined
    return vitaminPageSchema({
      id: vitamin.id,
      name: v.name,
      subtitle: v.subtitle,
  ...
  const crumbs = useMemo(() => {
    ...
    return breadcrumbSchema([
      { name: 'Home', url: 'https://vitamin.ai-biz.app/' },
      { name: 'Vitamins', url: 'https://vitamin.ai-biz.app/vitamins' },
      { name: v.name, url: `https://vitamin.ai-biz.app/vitamins/${id}` },
    ])
  }, [v, id])
  ...
  if (!v || !vitamin) {
    return (
      <div className="relative">
        <SEO title={seoTitle} description={seoDesc} lang={lang} />
        <Navigation />
        <main style={{ paddingTop: 60 }} className="min-h-[60vh] flex flex-col items-center justify-center px-6">
          ...
          <Link href="/vitamins" className="mt-6 px-6 py-2.5 rounded-full text-[14px] font-medium text-white" style={{ background: '#1a1a1a', fontFamily: "'Geist Sans', sans-serif" }}>
            {t('vd.back')}
          </Link>
        </main>
      </div>
  ...
  const currentIndex = vitamin ? vitamins.findIndex((vi) => vi.id === vitamin.id) : -1
  const prevVitamin = currentIndex > 0 ? vitamins[currentIndex - 1] : null
  const nextVitamin = currentIndex >= 0 && currentIndex < vitamins.length - 1 ? vitamins[currentIndex + 1] : null
      ...
      <main style={{ paddingTop: 60 }} ref={contentRef}>
        ...
        <section className="vd-hero w-full pt-16 pb-12 px-6" style={{ background: `linear-gradient(135deg, ${v.bgTint} 0%, #fafaf8 100%)`, borderBottom: `1px solid ${v.color}15` }}>
          <div className="content-max">
            <div className="flex items-center gap-2 mb-8">
              <Link href="/" className="text-[13px] text-[#4a4a4a] hover:text-[#1a1a1a] transition-colors" style={{ fontFamily: "'Geist Sans', sans-serif" }}>{t('nav.home')}</Link>
              ...
              <Link href="/vitamins" className="text-[13px] text-[#4a4a4a] hover:text-[#1a1a1a] transition-colors" style={{ fontFamily: "'Geist Sans', sans-serif" }}>{t('nav.vitamins')}</Link>
              <svg width="12" height="12" viewBox="0 0 16 16" fill="none" className="opacity-40"><path d="M6 12L10 8L6 4" stroke="#4a4a4a" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
              ...
              <div className="lg:col-span-2 space-y-8">
                  ...
                  <div className="vd-section rounded-xl p-8 bg-white" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
                    <h2 className="text-[#1a1a1a] font-semibold flex items-center gap-3" style={{ fontFamily: "'Geist Sans', sans-serif", fontSize: 20 }}>
                      <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: 'rgba(96, 165, 250, 0.12)' }}>
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#3b82f6" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h2M18 12h2M6.34 17.66l-1.41 1.41M19.07 4.93l-1.41 1.41"/></svg>
                      </div>
                      Sources
                    </h2>
                ...
                <div className="vd-section rounded-xl p-6 bg-white" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
                  <p className="text-[#4a4a4a]" style={{ fontFamily: "'Geist Sans', sans-serif", fontSize: 13, lineHeight: 1.6 }}>
                    <strong className="text-[#1a1a1a]">Reviewed for accuracy:</strong> Content is based on guidelines from the NIH Office of Dietary Supplements, Harvard T.H. Chan School of Public Health, and peer-reviewed medical literature.
                  </p>
                </div>
              </div>
                ...
                <div className="vd-section rounded-xl p-6 bg-white" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
                  ...
                  <div className="space-y-1">
                    {vitamins.map((vi) => (
                      <button
                        key={vi.id}
                        onClick={() => router.push(`/vitamins/${vi.id}`)}
        ...
        <section className="w-full py-8 px-6" style={{ background: '#fafaf8', borderTop: '1px solid rgba(0,0,0,0.06)' }}>
          <div className="content-max flex justify-between items-center">
            {prevVitamin ? (
              <button onClick={() => router.push(`/vitamins/${prevVitamin.id}`)} className="flex items-center gap-3 group text-left">
            ...
            {nextVitamin ? (
              <button onClick={() => router.push(`/vitamins/${nextVitamin.id}`)} className="flex items-center gap-3 group text-right">
                <div>
