...
import { howToSupplementGuideSchema } from '../lib/seoSchema'
  ...
  if (showResults) {
    return (
      <div className="relative">
        <SEO
          title={t('sg.results.title')}
          description={t('sg.results.sub')}
         
          lang={lang}
          schema={howToSupplementGuideSchema(lang as 'en' | 'ko')}
        />
        <Navigation />
                    ...
                    {expandedCard === rec.vitaminId && (
                      <div className="px-6 pb-6 pt-2 space-y-4">
                        ...
                        {rec.vitaminId.startsWith('vitamin-') && (
                          <div className="pt-2">
                            <Link href={`/vitamins/${rec.vitaminId}`} className="inline-flex items-center gap-2 text-[13px] font-medium transition-all hover:gap-3" style={{ fontFamily: "'Geist Sans', sans-serif", color: rec.color }}>
